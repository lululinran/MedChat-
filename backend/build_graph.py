...
if __name__ == "__main__":
    ...
    parser.add_argument('--password', type=str, default='supermew123', help='neo4j的密码')
    ...
    with open('backend/data/medical_new_2.json','r',encoding='utf-8') as f:
    ...
    # 保存关系 放到data下
    with open("backend/data/rel_aug.txt",'w',encoding='utf-8') as f:
        for rel in relationship:
            f.write(" ".join(rel))
            f.write('\n')

    if not os.path.exists('backend/data/ent_aug'):
        os.mkdir('backend/data/ent_aug')
    for k,v in all_entity.items():
        with open(f'backend/data/ent_aug/{k}.txt','w',encoding='utf8') as f:
            if(k!='疾病'):
